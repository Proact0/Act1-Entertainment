...
class WeatherService:
    ...
    def get_current_weather(self, nx: str = "60", ny: str = "126") -> Dict[str, Any]:
        ...
        closest_time = min(
            informations.keys(), key=lambda x: abs(int(x) - int(current_time))
        )
        ...
        return {
            ...
            "sky_condition": self.sky_code.get(
                int(current_weather.get("SKY", 1)), "맑음"
            ),
            "precipitation": self.pty_code.get(
                int(current_weather.get("PTY", 0)), "강수 없음"
            ),
