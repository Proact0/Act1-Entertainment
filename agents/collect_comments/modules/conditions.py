...
def has_more_posts(state: dict) -> str:
    """
    다음 게시물이 있는지에 따라 워크플로우를 계속 진행할지 종료할지 결정합니다.

    Args:
        state (dict): 워크플로우 상태. 'current_post_url' 키를 포함해야 합니다.

    Returns:
        str: 다음 노드의 이름 ('load_comments' 또는 '__end__')
    """
    return "load_comments" if state.get("current_post_url") else "__end__"
