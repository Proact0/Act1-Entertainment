...
def test_music_workflow_execution(music_workflow, mock_chatgpt):
    """Workflow 실행 테스트"""
    # 테스트용 입력 상태
    initial_state = {"query": "여름 바다를 주제로 한 가사", "response": []}
...
def test_music_workflow_error_handling(music_workflow, mock_chatgpt):
    ...
    initial_state = {"query": "에러 테스트", "response": []}
...
def test_music_workflow_different_queries(
    music_workflow, mock_chatgpt, test_input, expected
):
    """다양한 쿼리에 대한 테스트"""
    initial_state = {"query": test_input, "response": []}
